...
class Mm extends CI_Model
{
	...
	function get($table, $data = array(), $returnformat = 'rear')
	{
        ...
        */
		...
		if (!empty($data['or_where'])) {
			...
			} elseif ($data['or_where']) {
				$this->db->or_where("(" . $data['or_where'] . ")", null, FALSE);
			}
		}
		/*
        $data['where_in'] = array(
	...
	function save_ignore($table, $post = array())
	{
		$table = $this->db->dbprefix($table);
	...
	function save_all_ignore($table, $keys = '', $post = array())
	{
		if ($post) {
			$table = $this->db->dbprefix($table);
			if (is_array($post)) {
			...
			$insert_query = "INSERT IGNORE INTO `" . $table . "` (" . $keys . ") VALUES " . $post . "";
	...
	function save_all($table, $post = array())
	{
		if ($post) {
			$table = $this->db->dbprefix($table);
			$insert = $this->db->insert_batch($table, $post);
			return $insert;
		} else {
			return FALSE;
		}
